...
trainsplit <- 0.75
train_index <- sample(1:nrow(ff_foul2023), nrow(ff_foul2023)*trainsplit)

trainDF <- ff_foul2023[train_index,]
testDF <- ff_foul2023[-train_index,]


# Format data for xgboost
dtrain <- xgb.DMatrix(data = as.matrix(select(trainDF, -foul)), 
                      label = trainDF$foul)
dtest <- xgb.DMatrix(data = as.matrix(select(testDF, -foul)), 
                     label = testDF$foul)
...
ff_foul_model <- xgb.train(data = dtrain, 
                           verbose = 0,
